...
export function RotaBuilder({ eventId, initialData = null }: RotaBuilderProps) {
  ...
  if (isLoading) {
    return (
      <div className="v-empty">
        <p className="text-sm text-slate-500 dark:text-slate-400">Loading rota…</p>
      </div>
    );
  }
  ...
  if (!data) {
    return (
      <div className="v-empty">
        <p className="text-sm font-medium text-slate-900 dark:text-slate-100">Event not found</p>
