...
export function StaffShiftCard({ shift, compact = false }: { shift: StaffShift; compact?: boolean }) {
  return (
    <Link
      ...
      className="block rounded-2xl border border-slate-200 bg-white p-4 shadow-sm transition-colors hover:border-brand-200 hover:bg-brand-50/30 dark:hover:border-brand-900 dark:hover:bg-brand-950/20"
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-0.5 truncate text-sm text-muted-foreground">{shift.venueName}</p>
        </div>
      ...
      <div className="mt-4 space-y-2 text-sm text-foreground/90">
        ...
        {!compact && (
          <>
            ...
            {shift.notes && (
              <p className="rounded-lg bg-slate-50 px-3 py-2 text-sm text-muted-foreground dark:bg-slate-800/60">
                {shift.notes}
              </p>
            )}
          </>
