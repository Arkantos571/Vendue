...
export function EditTeamMemberForm({ member }: { member: MockTeamMember }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8">
      ...
      <section className="v-panel"><h2 className="text-base font-semibold text-foreground">Notes</h2><div className="mt-5"><Textarea id="notes" name="notes" rows={4} defaultValue={member.notes ?? ""} /></div></section>
      <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
        <Link href={`/dashboard/team/${member.id}`} className="inline-flex h-10 items-center justify-center rounded-lg border border-slate-300 bg-white px-4 text-sm font-medium text-foreground hover:bg-slate-50">Cancel</Link>
